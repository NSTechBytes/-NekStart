...
foreach ($skinName in $skinNames) {
    # Trim '#' from the skin name if present
    $originalSkinName = $skinName
    if ($skinName.StartsWith("#")) {
        $skinName = $skinName.TrimStart("#")
        Write-Host "Trimmed '#' from the skin name: $originalSkinName -> $skinName"
    }
    ...
    $destinationFolder = "C:\nstechbytes"
    $extractionFolder = "$destinationFolder\$originalSkinName"
    ...
    Write-Host "Downloaded $originalSkinName version $version to $destination"
    ...
    Write-Host "Extracted $originalSkinName to $extractionFolder"
    ...
    if (Test-Path -Path $skinsDirectory) {
        ...
        $skinToRemove = Join-Path -Path $skinsDirectory -ChildPath $originalSkinName
    ...
    $extractedSkinFolder = Join-Path -Path $extractionFolder -ChildPath "Skins\$originalSkinName"

    if (Test-Path -Path $extractedSkinFolder) {
        $destinationSkinPath = Join-Path -Path $skinsDirectory -ChildPath $originalSkinName
    ...
    if (Test-Path -Path $rainmeterPath) {
        Start-Process -FilePath $rainmeterPath
        Start-Sleep -Seconds 1
        if ($originalSkinName -eq "#NekStart") {
            Start-Process -FilePath $rainmeterExePath -ArgumentList "!ActivateConfig #NekStart\Startup Main.ini"
            Write-Host "Activated skin: $skinName"
        }
        else {
            Write-Host "Skipping activation for skin: $skinName"
        }
